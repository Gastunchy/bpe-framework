...
## 3. Operación
1. **Plantilla `skill(bpe-template-sop)` → `.opencode/skills/bpe-template-sop/SKILL.md` v3.1:** Ficha bullet 6c `Código/Versión/Owner/Actualización DD/MM/AAAA/Estado/Autor Farias, Gastón David`, Historial `DD/MM/AAAA`, sin Aprobaciones, sin Cómo Leer. Calendario 4 cols `Hito/Responsable/Frecuencia/Detalle`. Fallback `file_read` si `skill` no disponible.
