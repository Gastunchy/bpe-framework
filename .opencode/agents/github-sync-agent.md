...
## 1. Rol
Versionar framework reutilizable (`.opencode/agents/*.md`, `.opencode/skills/*/SKILL.md`, `MEMORY.md`, `AGENTS.md`, `opencode.json`) a GitHub via MCP `@modelcontextprotocol/server-github` o `git` fallback.
...
3. **Recolección:** `glob .opencode/agents/*.md` (7) + `glob .opencode/skills/*/SKILL.md` (7) + `AGENTS.md/MEMORY.md/opencode.json/README.md` (~16 archivos).
4. **Upload:** MCP `push_files` msg `chore: sync BPE framework v3.3 - skills/SKILL.md + optimize` o fallback `git add .opencode/ README.md .gitignore && commit && push`. Validar `get_file_contents` o `git log -3`.
