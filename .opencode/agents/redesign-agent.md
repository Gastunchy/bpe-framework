...
2. **SOP To-Be `skill(bpe-sop-tobe)` → `.opencode/skills/bpe-sop-tobe/SKILL.md` + `skill(bpe-template-sop)`:** Ficha 6c + Historial `v3.0` + RACI 1xA + RN `ID|Regla|Aplicación` + Proc `Actor/Sistema/Acción/Estado` oblig + Riesgos 3c + KPIs 5 filas LaTeX + SIPOC 4c + Anexos celda x celda.
